...
using TiledLib;
using FantasyEngine.Classes.Overworld.Maps;
...
namespace FantasyEngine.Classes.Overworld
{
	public abstract class Map : DrawableGameComponent
	{
		...
		/// <summary>
		/// Type of the map to call static data dynamicaly.
		/// </summary>
		public Type MapType;
		...
		public string BattleBackName = "battleback_grass";
				...
				foreach (MapObject obj in ((MapObjectLayer)_MapData.GetLayer(LAYER_NAME_NPC)).Objects)
				{
					...
					if (obj.Properties[PROP_NAME_NPC_TALK] != null)
						npc.Talking += (NPC.TalkingHandler)Delegate.CreateDelegate(typeof(NPC.TalkingHandler),
							MapType.GetMethod(obj.Properties[PROP_NAME_NPC_TALK].RawValue, BindingFlags.Public | BindingFlags.Static));

					if (obj.Properties[PROP_NAME_NPC_MOVE] != null)
						npc.Moving += (NPC.MovingHandler)Delegate.CreateDelegate(typeof(NPC.MovingHandler),
							MapType.GetMethod(obj.Properties[PROP_NAME_NPC_MOVE].RawValue, BindingFlags.Public | BindingFlags.Static));
					...
					switch (obj.Type)
					{
						...
						case TYPE_NAME_EVENT_ONENTER:
							...
							eve.OnEnter += (Event.OnEnterHandler)Delegate.CreateDelegate(typeof(Event.OnEnterHandler),
								MapType.GetMethod(obj.Properties[PROP_NAME_EVENT_ONENTER].RawValue, BindingFlags.Public | BindingFlags.Static));
							break;
					}
		...
		{
			Initialize();
			MapType = Type.GetType("FantasyEngine.Classes.Overworld.Maps." + mapName.Replace(" ", ""));
			Init(mapName);
			Offset = Player.GamePlayer.Hero.Position - Overworld.CAMERA_CENTER;
		...
		public static IMapFactory GetFactory(string mapName)
		{
			return Assembly.GetExecutingAssembly().CreateInstance("FantasyEngine.Classes.Overworld.Maps." + mapName.Replace(" ", "") + "Factory") as IMapFactory;
		}
	}
}
