...
{
	public sealed class VillageFactory : IMapFactory
	{
		public Map CreateMap(Game game)
		{
			return new Village(game);
		}
	}

	public sealed class Village : Map
	{
		...
		public static void Griswold_Talk(EventArgs e, NPC npc)
		{
			Thread thr = new Thread(
				delegate(object Data)
				{
					...
					List<BaseItem> shopBuy = new List<BaseItem>();
					shopBuy.Add(ItemManager.GetItem("Potion"));
					shopBuy.Add(ItemManager.GetWeapon("Dagger"));
					shopBuy.Add(ItemManager.GetWeapon("Long Sword"));
